...
public class Headingsreadout {

    private static final String TAG = "create audio";


    public static StringBuilder sortgeneral(int h) {
        int check1=0;
        int check2=0;
        int check3=0;
        int temp1;
...
//                        storetext.add(" Sub Heading " +" "+" " + baseApi.getUTF8Text());
        ...
        for (int m = 0; m < ff1.length; m++){
            Log.d(TAG, "loop 0");
            int finalM = m;
            if(intarrayh[h]==ff1[m])
            {
                baseApi.setImage(BitmapFactory.decodeFile("/sdcard/Any Reader/Headings/h" + ff1[finalM] + ".jpg"));
                storetext.add(" Under heading "+(h+1)+" Sub Heading is" +" "+" " + baseApi.getUTF8Text());
                for(int i=1;i<intarrayh.length;i++)
                    {
                        int finalM1=m+i;
                        if (intarrayh[h+i]==ff1[finalM+i])
                        {
                            baseApi.setImage(BitmapFactory.decodeFile("/sdcard/Any Reader/Headings/h" + ff1[finalM1] + ".jpg"));
                            storetext.add(" Sub Heading " +" "+" " + baseApi.getUTF8Text());
                        }
                        else if (IntStream.of(intarrayp).anyMatch(x -> x == (ff1[finalM1])) == true)
                        {
                            baseApi.setImage(BitmapFactory.decodeFile("/sdcard/Any Reader/Points/p" + ff1[finalM1] + ".jpg"));
                            storetext.add(" Under which point is " +" "+" " + baseApi.getUTF8Text());
                        }
                        else if (IntStream.of(intarrayb).anyMatch(x -> x == (ff1[finalM1])) == true)
                        {
                            baseApi.setImage(BitmapFactory.decodeFile("/sdcard/Any Reader/Para/b" + ff1[finalM1] + ".jpg"));
                            storetext.add(" Under which paragraph is" +" "+" " + baseApi.getUTF8Text());
